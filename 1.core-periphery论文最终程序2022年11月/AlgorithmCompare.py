...
def compare_algorithm(X_train, X_test, y_train, y_test, i):
    ...
    classifiers = [
        ...
        AdaBoostClassifier(),
        GaussianNB()]
        # CorePeriphery(num_class=2, in_rate=1.7)]
    ...
    for name, clf in zip(names, classifiers):
        ...
        cm_dic[str(i) + "_" + name + "_" + str(score)] = cm
        # score_dict[name] = score
        score_list.append(score)

    return score_list, cm_dic
...
if __name__ == '__main__':
    ...
    for i in range(times):
        print("times:", "第" + str(i) + "次")
        X_train, X_test, y_train, y_test = train_test_split(x, y, test_size=.2, stratify=y)
        X_train, X_test = data_preprcess(X_train, X_test)
        score_list, cm_aver = compare_algorithm(X_train, X_test, y_train, y_test, i)
