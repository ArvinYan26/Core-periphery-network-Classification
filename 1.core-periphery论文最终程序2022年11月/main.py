...
if __name__ == '__main__':
    ...
    times = [50]
    in_rate = 1.7     #节点图阈值,论文中的
    # in_rate = 3     #节点图阈值

    dict2_all = dict()
    dict2_all[str(in_rate)] = in_rate
    ever_time = None
    # for j in range(len(times)):
    # print("times[j]:", times[j])
    # ever_time = str(times[j])
    dict_ever = dict()
    acc_all = []
    for i in range(times[j]):
        print("times[j]:", str(times[j]) + "-" + str(i))
        # 每一次数据一样
        ...
        # 测试结果1.14，之前是最好的
        cp = CorePeriphery(num_class=2, in_rate=in_rate)  # 调用模块, 类别数
        # G_list, adj = cp.fit(x_train, y_train)
        cp.fit(x_train, y_train)
        acc = cp.score(x_test, y_test)
        acc = round(acc, 3)   #保留三位小数

        acc_all.append(acc)
        print("acc:", acc)
        # y_predict = cp.predict(x_test, y_test)
        # dr.draw_confusion_matrix(y_test, y_predict)


        mean_acc = np.mean(acc_all)
        # mean_acc = round(mean_acc, 3)
        dict_ever[str(times[j])] = times[j]
        dict_ever["mean_acc"] = mean_acc
        dict_ever["acc"] = acc_all

        print("mean_acc, acc_all:", mean_acc, acc_all)
        acc_ever.append(dict_ever)

    dict2_all["results"] = acc_ever
    ...
    # x_train, x_test = dr.data_preprcess(x_train, x_test)  # 数据归一化
    # # cp = CorePeriphery(num_class=2, in_rate=1.01)  #调用模块, 类别数
    # cp = CorePeriphery(num_class=2, in_rate=3)  #调用模块, 类别数
    # #G_list, adj = cp.fit(x_train, y_train)
    # cp.fit(x_train, y_train)
    # acc, y_predict = cp.score(x_test, y_test)
    # print("acc:", acc)
    # # dr.draw_confusion_matrix(y_test, y_predict)
